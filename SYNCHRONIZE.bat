...
set "folders=documents pictures videos music"

for %%F in (%folders%) do (
    set "folder_path=%user_path%\%%F"
    for /f "usebackq delims=" %%A in (`powershell -noprofile -command ^
        "if (Test-Path '!folder_path!') {(Get-ChildItem '!folder_path!' -Recurse -ErrorAction SilentlyContinue | Measure-Object -Property Length -Sum).Sum} else {0}"`) do (
        set "size_%%F=%%A"
    )
...
for /f "delims=" %%A in ('powershell -Command "[math]::Round(%size_data_usb% / 1GB, 2)"') do set gb=%%A

set size_data_usb_GB=%gb%
...
echo:
echo            1. Copy and rename the script ^(for example use user's name as scriptname^).
